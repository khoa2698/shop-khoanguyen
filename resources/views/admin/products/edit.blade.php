    ...
    <form action="" method="POST" enctype="multipart/form-data" id="formAddProduct">
        ...
        <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="parent_category_id" id="parent_category_id">
                            @foreach ($menus as $item)
                                <option value="{{$item->id}}" {{ $product->parent_category_id == $item->id ? 'selected' : ''}}>{{$item->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <input type="hidden" name="thumb" value="{{ $product->thumb }}" id="file_link">
            ...
            <div class="form-group">
                ...
                <div class="custom-control custom-radio">
                <input class="custom-control-input" value="0" type="radio" id="hidden" name="active" {{ $product->active == 0 ? 'checked=""' : ''}}>
